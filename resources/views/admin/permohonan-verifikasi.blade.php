...
<body class="bg-slate-200">
                ...
                <table class="min-w-[700px] w-full text-sm text-left rtl:text-right text-primary2">
                    <thead class="text-xs text-gray-700 uppercase bg-white">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">KTP</th>
                    ...
                    <tbody>
                        @forelse($pendingUsers as $user)
                            <tr class="bg-white border-b border-gray-200">
                                <th scope="row" class="px-4 py-4 font-medium text-primary2 whitespace-nowrap">{{ $user->nama }}</th>
                                <td class="px-4 py-4">{{ $user->email }}</td>
                                <td class="px-4 py-4">{{ $user->nip }}</td>
                                <td class="px-4 py-4">{{ $user->no_hp }}</td>
                                <td class="px-4 py-4"><img src="" alt="">{{ $user->foto_ktp }}</td>
                                <td class="px-4 py-4">{{ $user->role }}</td>
                                <td class="px-4 py-4">{{ $user->prodi }}</td>
                                <td class="px-4 py-4 space-x-2 flex whitespace-nowrap">
                                    <form action="{{ route('verifikasi.terima', $user->id) }}" method="POST" class="accept-form">
                                        @csrf
                                        <button type="submit" class="rounded-md hover:opacity-85 bg-green-500 flex p-2 font-semibold text-white">Accept</button>
                                    </form>
                                
                                    <form action="{{ route('verifikasi.tolak', $user->id) }}" method="POST" class="reject-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="rounded-md hover:opacity-85 bg-red-500 flex p-2 font-semibold text-white">Reject</button>
                                    </form>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="8" class="text-center py-4 text-gray-500">Belum ada Akun yang mendaftar.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <script>
        ...
        // SweetAlert2 for Reject button
        document.querySelectorAll('.reject-form').forEach(function(form) {
            form.addEventListener('submit', function(event) {
                event.preventDefault();
                Swal.fire({
                    title: 'Yakin ingin menolak akun ini?',
